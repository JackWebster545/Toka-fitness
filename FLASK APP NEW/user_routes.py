...
# pip install request and hashlib
import requests
import hashlib
...
def login():
    ...
    if request.method == "POST":
        ...
        found_user = db.queryDB("SELECT * FROM customers WHERE name = ?",[user])
...
def register():
    ...
    if request.method == "POST":
        ...
        result = db.queryDB("SELECT * FROM customers WHERE name = ? OR email = ?", [user,hashed_email])
        if result:
        ...
        db.updateDB("INSERT INTO customers (email, password, name) VALUES (?,?,?)", [hashed_email,hashed_password,user])
